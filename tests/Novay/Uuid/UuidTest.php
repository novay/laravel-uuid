...
class UuidTest extends TestCase
{
    public function testStaticGeneration()
    {
        ...
        $this->assertInstanceOf('Novay\Uuid\Uuid', $uuid);

        $uuid = Uuid::generate(3, 'novay.web.id', Uuid::NS_DNS);
        $this->assertInstanceOf('Novay\Uuid\Uuid', $uuid);

        $uuid = Uuid::generate(4);
        $this->assertInstanceOf('Novay\Uuid\Uuid', $uuid);

        $uuid = Uuid::generate(5, 'novay.web.id', Uuid::NS_DNS);
        $this->assertInstanceOf('Novay\Uuid\Uuid', $uuid);
    }
    ...
    public function testGenerationOfValidUuidViaValidator()
    {
        ...
        $this->assertTrue(Uuid::validate(Uuid::generate(3, 'novay.web.id', Uuid::NS_DNS)));

        $this->assertTrue(Uuid::validate(Uuid::generate(4)));

        $this->assertTrue(Uuid::validate(Uuid::generate(5, 'novay.web.id', Uuid::NS_DNS)));
    }
    ...
    public function testCorrectVersionUuid()
    {
        ...
        $uuidThree = Uuid::generate(3, 'novay.web.id', Uuid::NS_DNS);
        $this->assertEquals(3, $uuidThree->version);

        $uuidFour = Uuid::generate(4);
        $this->assertEquals(4, $uuidFour->version);

        $uuidFive = Uuid::generate(5, 'novay.web.id', Uuid::NS_DNS);
